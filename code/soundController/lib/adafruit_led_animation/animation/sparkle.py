...
class Sparkle(Animation):
    ...
    def __init__(
        self, pixel_object, speed, color, num_sparkles=1, name=None, mask=None, num_pixels=0, pixel_start=0
    ):
        ...
        self._num_pixels = len(pixel_object) if num_pixels == 0 else num_pixels
    ...
    def _set_color(self, color):
        ...
        for pixel in range(self._num_pixels):
            if self.pixel_object[pixel + self._pixel_start] == self._half_color:
                self.pixel_object[pixel + self._pixel_start] = half_color
            elif self.pixel_object[pixel + self._pixel_start] == self._dim_color:
                self.pixel_object[pixel + self._pixel_start] = dim_color
    ...
    def _random_in_mask(self):
        if len(self._mask) == 0:
            return random.randint(0, (self._num_pixels - 1))
    ...
    def draw(self):
        ...
        for pixel in self._pixels:
            self.pixel_object[pixel + self._pixel_start] = self._sparkle_color

    def after_draw(self):
        self.show()
        for pixel in self._pixels:
            self.pixel_object[(pixel % self._num_pixels) + self._pixel_start] = self._half_color
            if (pixel + 1) % self._num_pixels in self._mask:
                self.pixel_object[((pixel + 1) % self._num_pixels) + self._pixel_start] = self._dim_color
