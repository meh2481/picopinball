...
class ColorCycle(Animation):
    ...
    def __init__(self, pixel_object, speed, colors=RAINBOW, name=None, num_pixels=0, pixel_start=0):
        self.colors = colors
        self._pixel_start = pixel_start
        if num_pixels == 0:
            num_pixels = len(pixel_object)
    ...
    def draw(self):
        for i in range(self._num_pixels):
            self.pixel_object[i + self._pixel_start] = self.color
        next(self._generator)
