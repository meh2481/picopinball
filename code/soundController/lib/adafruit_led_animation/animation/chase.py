...
class Chase(Animation):
    ...
    def __init__(
        self, pixel_object, speed, color, size=2, spacing=3, reverse=False, name=None, num_pixels=0, pixel_start=0
    ):
        self._size = size
        ...
        self._num_repeats = ceil(self._num_pixels / self._repeat_width)
        self._overflow = self._num_pixels % self._repeat_width
    ...
    def draw(self):
        ...
        for i in range(self._num_pixels):
            self.pixel_object[i + self._pixel_start] = next(colorgen)

        if self.draw_count % self._num_pixels == 0:
